...
public class SpellCheckerSettings implements PersistentStateComponent<Element> {
    ...
    public static SpellCheckerSettings getInstance(Project project) {
        return project.getInstance(SpellCheckerSettings.class);
    }
    ...
    public void loadState(@Nonnull Element element) {
        ...
        try {
            // bundled
            int bundledDictionariesSize = Integer.valueOf(element.getAttributeValue(BUNDLED_DICTIONARIES_ATTR_NAME));
            ...
            // user
            int foldersSize = Integer.valueOf(element.getAttributeValue(FOLDERS_ATTR_NAME));
            for (int i = 0; i < foldersSize; i++) {
                myDictionaryFoldersPaths.add(element.getAttributeValue(FOLDER_ATTR_NAME + i));
            }
            int scriptsSize = Integer.valueOf(element.getAttributeValue(DICTIONARIES_ATTR_NAME));
